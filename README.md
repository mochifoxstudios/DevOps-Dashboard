...
This dashboard was generated by Claude Design from a single multi-turn chat (`docs/design-chat.md`), then hardened in nine passes:
...
6. **AI & GitHub closed loop (Phase 5)** — pluggable LLM providers (Ollama / OpenAI / Anthropic / Bedrock), automatic root-cause enrichment of LogWatchdog drafts, AI-narrated Context-Snap diffs, one-click "File on GitHub", and a hash-chained, tamper-evident AI audit log.
7. **Security hardening** — loopback-by-default bind, localhost-only CORS, symlink-resolving workspace boundary, log-extension-gated tailing, per-hop SSRF redirect validation, and corrected redaction accounting — each landed test-first (unit + smoke). See [`docs/superpowers/plans/2026-06-03-agent-security-hardening.md`](docs/superpowers/plans/2026-06-03-agent-security-hardening.md).
8. **V1.00.1 (research-driven)** — quality + the compliance moat. Made Log-Tail filtering real (engine-level, level selector, Export/Clear), stood up the first frontend test harness, surfaced the tamper-evident **Audit Log** as a first-class view, added a one-switch **Air-Gapped Mode** with a zero-egress self-test, universal **.dvops.json** backup/import, snapshot/draft search, a richer Compare view, real command-palette actions, an a11y pass, and agent robustness (EADDRINUSE recovery, `AGENT_STATE_DIR`, queue caps). Planned from a 6-agent research workflow — see [`docs/superpowers/plans/2026-06-03-v1-00-1-improvements.md`](docs/superpowers/plans/2026-06-03-v1-00-1-improvements.md).
9. **V1.00.2** — a desktop app (Electron wrapper running the agent as a sidecar), a Node-compat fix (chokidar pinned to CJS so the agent runs on Node 18–20.18 / Electron), and further improvements: **Ed25519-signed** audit records (externally verifiable, with `/api/agent/audit/pubkey`), **lockfile** parsing in Dependency Map (`package-lock.json` / `yarn.lock` / `pnpm-lock.yaml`), a real **manifest-change watcher**, and real diagnostics on the login screen. CI + branch protection added. See [`docs/superpowers/plans/2026-06-04-v1-00-2-improvements.md`](docs/superpowers/plans/2026-06-04-v1-00-2-improvements.md).
