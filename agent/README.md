...
- **Ed25519 signatures (externally verifiable).** Every record is signed with a per-install Ed25519 key; `verify()` checks both the hash chain and each signature (`reason: 'signature'` when a signature fails — this catches tampering the chain alone misses, e.g. editing the latest record). Fetch the public key at `GET /api/agent/audit/pubkey` to verify an exported log offline. **Honest caveat:** the private key (`agent/.audit-key.pem`) lives beside the log under the same local-read trust model as `.salt`, so signing proves authenticity to an *external* party holding only the public key — it does **not** defend against a local attacker who can read the private key.
